...
async function refreshProducts() {
    ...
    try {
        const res = await fetch(
            GAS_URL + "?action=getInventoryProducts"
        );
        ...
        if (!response.success) {
            throw new Error(
                response.error || "Failed to load inventory products"
            );
        }
...
function updateInventoryHeader(products) {
    ...
    document.getElementById("totalProducts").innerText = Number(totalProducts).toLocaleString("en-IN");;
    document.getElementById("totalValue").innerText = "₹" + Number(totalValue).toLocaleString("en-IN");;
}
...
async function submitSale(productId) {
    ...
    const quantity =
        Number(document.getElementById("soldQuantity").value);

    const team =
        document.getElementById("soldMarketingTeam").value;
    ...
    try {
        ...
        if (!response.success) {
            throw new Error(
                response.error || "Failed to record sale"
            );
        }
...
async function refreshDashboard() {
    ...
    try {
        const res = await fetch(
            GAS_URL + "?action=getDashboardData"
        );
        ...
        if (!response.success) {
            throw new Error(
                response.error || "Failed to refresh dashboard"
            );
        }
...
function showUpdateExistingProductForm() {
    ...
    const section =
        document.getElementById(
            "inventory_update_existing_product"
        );
...
async function updateInventoryAddNewProduct() {
    const submitButton = document.getElementById("inventory_add_new_product_btn_submit")
    ...
    const amount =
        Number(
            document.getElementById(
                "inventory_add_new_product_amount"
            ).value
        );
    ...
    const quantity =
        Number(
            document.getElementById(
                "inventory_add_new_product_quantity"
            ).value
        );

    const file =
        document.getElementById(
            "inventory_add_new_product_photo"
        ).files[0];
...
function switchTab(tab) {
    ...
    const expensesView =
        document.getElementById("expensesView");

    const salesView =
        document.getElementById("salesView");
...
async function loadExpenseHeader(month) {
    ...
    try {
        ...
        if (!response.success) {
            throw new Error(
                response.error || "Failed to load expense summary"
            );
        }
...
async function loadSalesData(month) {
    ...
    if (salesCache[month]) {
        ...
        updateSalesHeader(
            salesCache[month].summary
        );

        renderSalesList(
            salesCache[month].list
        );
...
function hideSalesLoading() {
    ...
    document.getElementById("salesCommission")
        .classList.remove("skeleton");

    document.getElementById("salesProfit")
        .classList.remove("skeleton");
}
...
function clearInventoryAddNewProductForm() {
    ...
    document.getElementById("inventory_add_new_product_manufacturing_cost").value = "300";
